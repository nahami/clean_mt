...
knmi_aws2 <- merge(knmi_aws[,c(1,3,4,5,6,7,8)],wow_rur, by = 'datetime', all = TRUE)
knmi_aws2 <- knmi_aws2[,c(1,8,2,3,4,5,6,7)]

# Uncomment next line to choose wow as rural reference
# knmi_aws <- knmi_aws2 
names(knmi_aws2) <- names(knmi_aws)
urb_obs$datetime <-  as.character(urb_obs$datetime)
# knmi_aws$datetime <- as.POSIXct(knmi_aws$datetime)
...
lin_mse <- sqrt(MSE(matrixTestY,matrixTestX,lmFit))
bst_mse <- sqrt(MSE(matrixTestY,matrixTestX,bst))
